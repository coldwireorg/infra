    ...
    task "cw-auth-hydra-migrate" {
      ...
      template {
        data = <<EOH
          ...
          DSN=postgres://postgres:{{ with secret "services/data/cw-auth" }}{{ .Data.data.hydra_db_password }}{{ end }}@{{ index env["NOMAD_IP_cw-auth-hydra-database"] }}:{{ env "URLS_LOGIN" }}/hydra
        EOH
